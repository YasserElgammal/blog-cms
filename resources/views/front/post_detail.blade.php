            ...
            @auth
                ...
                <div class="w-full flex text-center md:text-left md:flex-col shadow bg-white mt-10 mb-10 p-6">
                    ...
                    <form method="POST" action="{{ route('post.comment', $post) }}">
                        @csrf
                        <textarea class="w-full p-2.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg" name="body"
                            cols="30" rows="10" placeholder="Write your comment here .."></textarea>
                        <button type="submit" class="mt-2 py-2 px-4 bg-green-500 hover:bg-green-700 hover:text-white">Submit
                            Comment</button>
                    </form>
                </div>
            @else
